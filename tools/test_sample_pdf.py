...
def main():
    ...
    z_vals = z_vals.reshape(1, ray_samples).repeat(2, 1)

    weights = torch.ones_like(z_vals) * 0.1
    ...
    sampled_z = sample_pdf_uni(z_vals, weights, 64, 1.0)

    plt.plot(z_vals[0].numpy(), weights[0].numpy())
